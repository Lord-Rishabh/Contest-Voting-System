...
const Voting = () => {
    ...
    const handleSubmit = (e) => {
        ...
        const endTimeInSeconds = new Date(contestDetails.endTime).getTime() / 1000;
        console.log('Submitted contest details:', { ...contestDetails, endTimeInSeconds });
        // Reset form fields
        setContestDetails({
            contestName: '',
            endTimeInSeconds: 0
                    ...
                    {showCreateContestForm && (
                        <div className="absolute top-0 left-0 w-full mt-12 bg-gray-900 bg-opacity-75 p-4 md:p-12">
                            ...
                            <form onSubmit={handleSubmit} className="space-y-4">
                                ...
                                <label htmlFor="endTime" className="block">End Time (in seconds):</label>
                                <input type="number" id="endTime" name="endTime" value={contestDetails.endTimeInSeconds} onChange={handleEndTimeChange} min="0" className="w-full border rounded-md py-2 px-3 bg-gray-900 text-white" />
