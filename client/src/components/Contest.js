        ...
        <div className="container mx-auto px-4 py-8 text-white">
            ...
            {contests.map(contest => (
                <div key={contest.id} className="mb-8">
                    ...
                    <p className="text-gray-400 mb-4">End Time: {new Date(contest.endTime).toLocaleString()}</p>
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                        {contest.entries.map(entry => (
                            <div key={entry.id} className="bg-gray-800 rounded-md p-4 shadow-lg">
                                <h3 className="text-lg font-semibold mb-2">{entry.name}</h3>
                                <p className="text-gray-300">Total Votes: {entry.votes}</p>
                                <button onClick={() => handleVote(contest.id, entry.id)} className="mt-4 bg-purple-600 text-white px-4 py-2 rounded-md hover:bg-purple-700">Vote</button>
                            </div>
                        ))}
                    </div>
                    <div className="mt-4">
                        ...
                        <p className="text-gray-300">
                            {contest.entries.length > 0 && 
                                contest.entries.reduce((max, entry) => (entry.votes > max.votes ? entry : max)).name}
                        </p>
                    </div>
                </div>
            ))}
